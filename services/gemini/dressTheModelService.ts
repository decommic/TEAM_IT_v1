...
export async function generateDressedModelImage(
    ...
    // Assemble all parts for the API call in the correct order.
    // The explicit type annotation ensures `allParts` can hold different part types.
    const allParts: object[] = [clothingImagePart, modelImagePart];
    if (referenceImagePart) {
        allParts.push(referenceImagePart);
    }
    allParts.push(...historicalImageParts);
    allParts.push(textPart);

    try {
        console.log("Attempting to generate dressed model image...");
