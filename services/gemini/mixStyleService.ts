...
export async function mixImageStyle(contentImageDataUrl: string, styleImageDataUrl: string, options: MixStyleOptions): Promise<{ resultUrl: string; finalPrompt: string; }> {
    try {
        ...
        const promptParts = [
            'Bạn là một nghệ sĩ AI chuyên nghiệp. Nhiệm vụ của bạn là vẽ lại "Ảnh Nội Dung" được cung cấp theo một phong cách nghệ thuật đã được phân tích.',
            '**YÊU CẦU CỐT LÕI (ƯU TIÊN TUYỆT ĐỐI):**',
            '1. **Bảo toàn Nội dung:** Bức ảnh kết quả PHẢI giữ nguyên 100% kích thước, bố cục, chủ thể, và các đối tượng chính từ "Ảnh Nội Dung". TUYỆT ĐỐI không được thêm, bớt, hay thay đổi các yếu tố cốt lõi này.',
            `2. **Áp dụng Phong cách:** Áp dụng phong cách được mô tả chi tiết sau đây lên toàn bộ "Ảnh Nội Dung": "${styleDescription}"`
        ];
