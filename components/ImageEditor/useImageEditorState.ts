...
*/
// FIX: Import specific types from React to resolve namespace errors.
...
export const useImageEditorState = (
    ...
    // FIX: Add missing ref for tracking state at the start of an interaction.
    ...
    // FIX: Define beginInteraction to snapshot the state before a change.
    const beginInteraction = useCallback(() => {
        interactionStartHistoryState.current = captureState();
    }, [captureState]);
    ...
    const getPointerInView = (e: MouseEvent | TouchEvent<HTMLCanvasElement | HTMLDivElement>) => {
    ...
    const getCanvasCoords = useCallback((e: MouseEvent | TouchEvent<HTMLCanvasElement | HTMLDivElement>) => {
    ...
    const handleActionStart = (e: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
        e.preventDefault();
        ...
        const nativeEvent = (e as any).nativeEvent as globalThis.MouseEvent;
    ...
    const handleCanvasMouseMove = (e: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
        e.preventDefault();
    ...
    const handleActionEnd = (e: MouseEvent<HTMLDivElement> | TouchEvent<HTMLDivElement>) => {
        e.preventDefault();
    ...
    const handleApplyCrop = useCallback(() => {
        ...
        const image = sourceImageRef.current;
        const previewCanvas = previewCanvasRef.current;
        if (cropSelection.width < 1 || cropSelection.height < 1) {
            handleCancelCrop();
            return;
        }
        const scaleX = image.naturalWidth / previewCanvas.width;
        ...
        const sWidth = cropSelection.width * scaleX;
        const sHeight = cropSelection.height * scaleY;
        const cropCanvas = document.createElement('canvas');
        cropCanvas.width = sWidth;
        cropCanvas.height = sHeight;
        const cropCtx = cropCanvas.getContext('2d');
        if (!cropCtx) {
            console.error("Could not get context for cropping.");
            return;
        }
        cropCtx.drawImage(image, sx, sy, sWidth, sHeight, 0, 0, sWidth, sHeight);
        const newDataUrl = cropCanvas.toDataURL('image/png');
        ...
        restoreState(postCropState);
        handleCancelCrop();
    }, [cropSelection, sourceImageRef, previewCanvasRef, commitState, handleCancelCrop, pushHistory, restoreState, brushHardness, brushOpacity]);
    ...
    const handleRemoveBackground = useCallback(async () => {
        ...
        try {
            ...
            const newSnapshot: EditorStateSnapshot = { ...captureState(), imageUrl: resultUrl, drawingCanvasDataUrl: null };
    ...
    }, [internalImageUrl, captureState, pushHistory]);
    ...
    const handleAiEdit = useCallback(async () => {
        if (!aiEditPrompt || !internalImageUrl) return;
        setIsLoading(true);
        try {
            const canvasToEdit = await getFinalImage();
            if (!canvasToEdit) throw new Error("Could not prepare image for AI editing.");
            const resultUrl = await editImageWithPrompt(canvasToEdit, aiEditPrompt);
            const newSnapshot: EditorStateSnapshot = { ...captureState(), imageUrl: resultUrl, drawingCanvasDataUrl: null, };
            setInternalImageUrl(resultUrl);
             if (drawingCanvasRef.current) {
                drawingCanvasRef.current.getContext('2d')?.clearRect(0,0, drawingCanvasRef.current.width, drawingCanvasRef.current.height);
    ...
    }, [aiEditPrompt, internalImageUrl, getFinalImage, captureState, pushHistory]);
    
    const invertSelection = useCallback(() => {
        if (selectionStrokes.length > 0) setIsSelectionInverted(prev => !prev);
    }, [selectionStrokes]);

    const fillSelection = useCallback(() => {
        if (!isSelectionActive || !selectionPath || !drawingCanvasRef.current) return;
        const ctx = drawingCanvasRef.current.getContext('2d');
        if (!ctx) return;
        beginInteraction();
        ctx.save();
        ...
        ctx.fill(selectionPath, 'nonzero');
        ctx.restore();
        commitState();
    }, [isSelectionActive, selectionPath, brushColor, brushOpacity, beginInteraction, commitState]);

    const deleteImageContentInSelection = useCallback(() => {
        if (!isSelectionActive || !selectionPath || !sourceImageRef.current) return;
        const image = sourceImageRef.current;
        ...
        const scaleX = newCanvas.width / canvasDimensions.width;
        const scaleY = newCanvas.height / canvasDimensions.height;
        ...
        const newSnapshot: EditorStateSnapshot = { ...captureState(), imageUrl: newDataUrl, drawingCanvasDataUrl: null };
        setInternalImageUrl(newDataUrl);
        if (drawingCanvasRef.current) {
            drawingCanvasRef.current.getContext('2d')?.clearRect(0, 0, drawingCanvasRef.current.width, drawingCanvasRef.current.height);
        }
        pushHistory(newSnapshot);
        deselect();
    }, [isSelectionActive, selectionPath, sourceImageRef, canvasDimensions, captureState, pushHistory, deselect, beginInteraction]);
    
    const handleFileSelected = (e: ChangeEvent<HTMLInputElement>) => handleFileUpload(e, setupNewImage);
    const handleGallerySelect = (url: string) => setupNewImage(url);
    const handleWebcamCapture = (url: string) => setupNewImage(url);
    const handleCreateBlank = () => {
        ...
        setupNewImage(blankCanvas.toDataURL());
    };
    ...
    return {
        ...
        saturation, setSaturation, vibrance, setVibrance, hue, setHue,
        grain, setGrain, clarity, setClarity, dehaze, setDehaze, blur, setBlur,
        rotation, setRotation, flipHorizontal, setFlipHorizontal, flipVertical, setFlipVertical, isInverted, setIsInverted,
        colorAdjustments, setColorAdjustments,
        // UI
        openSection, setOpenSection,
        isGalleryPickerOpen, setIsGalleryPickerOpen,
        isWebcamModalOpen, setIsWebcamModalOpen,
        activeColorTab, setActiveColorTab,
        isShowingOriginal, setIsShowingOriginal,
        // Tools
        activeTool, setActiveTool, handleToolSelect,
        brushSize, setBrushSize, brushHardness, setBrushHardness,
        brushOpacity, setBrushOpacity, brushColor, setBrushColor,
        isDrawing, setIsDrawing, cursorPosition, setCursorPosition, isCursorOverCanvas, setIsCursorOverCanvas,
        aiEditPrompt, setAiEditPrompt,
        // Crop
        cropSelection, setCropSelection, cropAspectRatio, setCropAspectRatio,
        cropAction, setCropAction, hoveredCropHandle, setHoveredCropHandle,
        perspectiveCropPoints, setPerspectiveCropPoints, hoveredPerspectiveHandleIndex, setHoveredPerspectiveHandleIndex,
        // Selection
        interactionState, setInteractionState, selectionStrokes, setSelectionStrokes, isSelectionInverted, setIsSelectionInverted,
        penPathPoints, setPenPathPoints, currentPenDrag, setCurrentPenDrag, marqueeRect, setMarqueeRect,
        ellipseRect, setEllipseRect, featherAmount, setFeatherAmount,
        isSelectionActive, selectionPath,
        // Pan & Zoom
        panX, panY, scale, zoomDisplay, isSpacePanning, setIsSpacePanning,
        // Refs
        previewCanvasRef, drawingCanvasRef, overlayCanvasRef, tempDrawingCanvasRef, currentDrawingPointsRef,
        // Actions
        setupNewImage, handleFile, handleFileSelected, handleGallerySelect, handleWebcamCapture, handleCreateBlank,
        getFinalImage,
        handleUndo, handleRedo, commitState, resetAll, handleApplyAllAdjustments,
        handleActionStart, handleCanvasMouseMove, handleActionEnd,
        handleCancelCrop, handleApplyCrop,
        handleCancelPerspectiveCrop, handleApplyPerspectiveCrop,
        // Magic tools
        handleRemoveBackground, handleInvertColors, handleAiEdit,
        // Selection tools
        deselect, invertSelection, fillSelection, deleteImageContentInSelection, handleApplyAdjustmentsToSelection,
        handleRotateCanvas,
    };
};

// FIX: Export the ImageEditorState type so it can be used in other components.
