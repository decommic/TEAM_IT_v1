...
const PhotoRestoration: React.FC<PhotoRestorationProps> = (props) => {
    ...
    const renderSelect = (id: keyof PhotoRestorationState['options'], label: string, optionList: string[]) => (
        <div>
            <label htmlFor={id as string} className="block text-left base-font font-bold text-lg text-neutral-200 mb-2">{label}</label>
            <select id={id as string} value={appState.options[id] as string} onChange={(e) => handleOptionChange(id, e.target.value)} className="form-input">
