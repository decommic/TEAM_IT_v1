...
const Placeholder = ({ type = 'person' }: { type?: 'person' | 'architecture' | 'clothing' | 'magic' | 'style' }) => {
    const getIcon = () => {
        switch (type) {
            case 'person':
                return <PlaceholderPersonIcon className="w-full h-full" />;
            case 'architecture':
                return <PlaceholderArchitectureIcon className="w-full h-full" />;
            case 'clothing':
                return <PlaceholderClothingIcon className="w-full h-full" />;
            case 'magic':
                return <PlaceholderMagicIcon className="w-full h-full" />;
            case 'style':
                return <PlaceholderStyleIcon className="w-full h-full" />;
            default:
                return <PlaceholderPersonIcon className="w-full h-full" />;
        }
    };
    ...
    return (
        <div className={cn("polaroid-card", isClickable && "cursor-pointer")} onClick={handleClick}>
            <div className={cn(
                ...
                {status === 'done' && !mediaUrl && <Placeholder type={placeholderType as 'person' | 'architecture' | 'clothing' | 'magic' | 'style'} />}
