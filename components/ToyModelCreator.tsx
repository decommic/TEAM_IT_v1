...
const ToyModelCreator: React.FC<ToyModelCreatorProps> = (props) => {
    ...
    const CONCEPTS_DATA = useMemo(() => {
        ...
        // Guard against race condition where translations haven't loaded yet.
        if (typeof concepts !== 'object' || concepts === null || !concepts.desktop_model) {
            return {}; 
        }
        return {
          desktop_model: {
            ...
            options: [
              ...
              { 
                ...
                getChoices: (options: ToyModelCreatorState['options']) => (
                  options.computerType.toLowerCase().includes(concepts.desktop_model.tabletIdentifier) 
                ...
                <ResultsView
                    ...
                    <motion.div className="w-full md:w-auto flex-shrink-0" key="generated-toy" initial={{ opacity: 0, scale: 0.5, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0, rotate: 0 }} transition={{ type: 'spring', stiffness: 80, damping: 15, delay: 0.15 }}>
                        <ActionablePolaroidCard
                            type="output"
                            caption={(currentConceptData as any)?.name ?? t('common_result')} status={isLoading ? 'pending' : (appState.error ? 'error' : 'done')} mediaUrl={appState.generatedImage ?? undefined} error={appState.error ?? undefined}
